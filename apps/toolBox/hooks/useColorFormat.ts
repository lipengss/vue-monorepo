...
export function useColorFormat() {
	...
	const hexToRgb = (str: string): any => {
		...
		let reg = /^\#?[0-9A-Fa-f]{6}$/;
		if (!reg.test(str)) {
			ElMessage.warning('输入的HEX有误，请重新输入');
			return '';
		}
		str = str.replace('#', '');
		hexs = str.match(/../g);
	...
	const rgbToHex = (r: any, g: any, b: any): string => {
		...
		if (!reg.test(r) || !reg.test(g) || !reg.test(b)) {
			ElMessage.warning('输入的rgb颜色值有误，请重新输入');
			return '';
		}
	...
	const getDarkColor = (color: string, level: number): string => {
		...
		let rgb = useColorFormat().hexToRgb(color);
		for (let i = 0; i < 3; i++) rgb[i] = Math.floor(rgb[i] * (1 - level));
		return useColorFormat().rgbToHex(rgb[0], rgb[1], rgb[2]);
	};
